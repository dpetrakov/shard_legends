...
func TestReservationConstraintViolation(t *testing.T) {
	...
	// Мок для получения operation type mapping - используем Maybe() на случай если операция прерывается раньше
	...
	// Мок для коллекций и качества (по умолчанию) - используем Maybe() поскольку они могут не вызываться при ранней ошибке
	classifierRepo.On("GetCodeToUUIDMapping", ctx, models.ClassifierCollection).Return(map[string]uuid.UUID{}, nil).Maybe()
	classifierRepo.On("GetCodeToUUIDMapping", ctx, models.ClassifierQualityLevel).Return(map[string]uuid.UUID{}, nil).Maybe()
