...
func (dbc *dailyBalanceCreator) CreateDailyBalance(ctx context.Context, req *DailyBalanceRequest) (*models.DailyBalance, error) {
	...
	// Normalize target date to end of day UTC
	targetDate := req.TargetDate.UTC().Truncate(24*time.Hour).Add(24*time.Hour - time.Second)

	// Check if daily balance already exists for target date
	existingBalance, err := dbc.deps.Repositories.Inventory.GetDailyBalance(
		ctx, req.UserID, req.SectionID, req.ItemID, req.CollectionID, req.QualityLevelID, targetDate,
	)
	...
	// Calculate the end of target date
	endOfTargetDate := targetDate

	// Get all operations from fromDate to end of target date
	...
	for _, op := range operations {
		...
		if op.CreatedAt.Before(endOfTargetDate) || op.CreatedAt.Equal(endOfTargetDate) {
