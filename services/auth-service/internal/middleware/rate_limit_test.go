...
func TestRateLimiter_Allow(t *testing.T) {
	...
	rl := NewRateLimiter(2, time.Minute, logger, testMetrics)
	defer rl.Close()
...
func TestRateLimiter_Middleware_Success(t *testing.T) {
	...
	rl := NewRateLimiter(10, time.Minute, logger, testMetrics)
	defer rl.Close()
...
func TestRateLimiter_Middleware_RateLimited(t *testing.T) {
	...
	rl := NewRateLimiter(1, time.Minute, logger, testMetrics)
	defer rl.Close()
...
func TestRateLimiter_ConfigurableWindow(t *testing.T) {
	gin.SetMode(gin.TestMode)

	logger := slog.New(slog.NewTextHandler(os.Stderr, &slog.HandlerOptions{Level: slog.LevelError}))
	testMetrics := createTestRateLimitMetrics()

	// Create rate limiter with 3 requests per 30 seconds
	rl := NewRateLimiter(3, 30*time.Second, logger, testMetrics)
	defer rl.Close()
