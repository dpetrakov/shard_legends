...
func TestTaskService_StartProduction_Idempotency(t *testing.T) {
	mockTaskRepo := new(MockTaskRepository)
	mockRecipeRepo := new(MockRecipeRepository)
	
	// Create minimal service with mocks needed for idempotency test
	taskService := &TaskService{
		taskRepo:   mockTaskRepo,
	...
	// Mock existing task (idempotency check)
	existingTask := models.ProductionTask{
		ID:       uuid.New(),
		UserID:   userID,
		RecipeID: recipeID,
	...
	existingTasks := []models.ProductionTask{existingTask}
	
	// Mock the GetUserTasks call for idempotency check
	mockTaskRepo.On("GetUserTasks", ctx, userID, []string{
	...
	// Execute
	result, err := taskService.StartProduction(ctx, userID, request)
	...
	// No recipe lookup or task creation should have occurred
	mockTaskRepo.AssertExpectations(t)
...
func (m *MockRecipeRepository) GetRecipeUsageStats(ctx context.Context, userID uuid.UUID, recipeID uuid.UUID, limitType string, limitObject string, targetItemID *uuid.UUID, periodStart, periodEnd time.Time) (*models.RecipeUsageStats, error) {
	args := m.Called(ctx, userID, recipeID, limitType, limitObject, targetItemID, periodStart, periodEnd)
	return args.Get(0).(*models.RecipeUsageStats), args.Error(1)
}
...
// Test edge case: idempotency check failure
func TestTaskService_StartProduction_IdempotencyCheckFailure(t *testing.T) {
	mockTaskRepo := new(MockTaskRepository)
	...
	// Mock failure in GetUserTasks
	checkError := errors.New("database connection failed")
	mockTaskRepo.On("GetUserTasks", ctx, userID, []string{
		models.TaskStatusDraft,
		models.TaskStatusPending,
		models.TaskStatusInProgress,
	}).Return([]models.ProductionTask{}, checkError)
	...
	assert.Error(t, err)
	assert.Nil(t, result)
	assert.Contains(t, err.Error(), "failed to check existing tasks")
