...
**Критерии выполнения:**
- [x] **TaskRepository**: Доработать методы CreateTask, GetUserTasks, UpdateTaskStatus в `internal/storage/task_repository.go`
- [x] **Эндпоинт GET /factory/queue**: HTTP handler для получения очереди заданий пользователя
- [x] **Эндпоинт POST /factory/start**: HTTP handler для создания нового производственного задания
- [x] **Эндпоинт GET /factory/completed**: HTTP handler для получения завершенных заданий
- [x] **Предрасчет результатов**: Алгоритм определения выходных предметов при создании задания
- [x] **Интеграция с Inventory Service**: Резервирование предметов через API inventory-service
- [x] **Управление производственными слотами**: Проверка доступности слотов и распределение заданий
- [x] **Система модификаторов**: Применение ускорителей и бонусов при создании задания
...
- [x] **Обработка ошибок**: Корректная обработка недостаточности предметов и превышения лимитов
...
**Критерии выполнения:**
- [x] **ModifierService**: Сервис для применения различных типов модификаторов
- [x] **Ускорители-предметы**: Обработка модификаторов из предметов инвентаря пользователя
- [x] **Пользовательские модификаторы**: Интеграция с user-service для получения VIP, уровня, достижений
- [ ] **Событийные модификаторы**: (TBD - будет реализовано в следующих версиях, после реализации Event Service)
- [x] **Автоматические модификаторы**: Клановые бонусы и серверные бафы
- [x] **Алгоритм предрасчета**: Определение результата производства в момент создания задания
- [x] **Случайная генерация**: Выбор предметов по вероятностям с учетом групп
- [x] **Наследование коллекций**: Передача коллекций и качества от входных к выходным предметам
- [x] **Фиксация результата**: Сохранение предрасчитанного результата в task_output_items
- [x] **Логирование модификаторов**: Сохранение примененных модификаторов для аудита
...
- Интеграция с event-service (TBD - в будущих версиях)
...
**Критерии выполнения:**
- [x] **Эндпоинт POST /factory/claim**: HTTP handler для получения результатов завершенных заданий
- [x] **Эндпоинт POST /factory/cancel**: HTTP handler для отмены заданий в статусе pending
- [x] **ClaimService**: Бизнес-логика для claim операций с валидацией статусов
- [x] **Интеграция с Inventory Service**: 
...
- [x] **Обновление статусов**: Изменение статуса задания с completed на claimed
- [x] **Освобождение слотов**: Логика освобождения производственных слотов после claim
- [x] **Атомарность операций**: Обеспечение консистентности между production-service и inventory-service
- [x] **Обработка ошибок**: Откат операций при сбоях в inventory-service
- [x] **Пакетный claim**: Поддержка claim всех готовых заданий одним запросом
- [x] **Аудит операций**: Логирование всех claim и cancel операций
...
**Критерии выполнения:**
- [x] **TaskService** - тесты для основной бизнес-логики:
  - [x] `StartProduction` - создание производственных заданий
  - [x] `ClaimTaskResults` - получение результатов завершенных заданий
  - [x] `CancelTask` - отмена заданий в статусе pending
  - [x] `GetUserQueue` - получение очереди заданий
  - [x] `GetCompletedTasks` - получение завершенных заданий
  - [x] Валидация доступа и статусов заданий
  - [x] Обработка ошибок интеграции с внешними сервисами
- [x] **ProductionCalculator** - тесты для расчетов производства:
  - [x] `PrecalculateProduction` - предрасчет с применением модификаторов
  - [x] `CalculateOutputItems` - расчет выходных предметов по вероятностям
  - [x] `GetModifiedProductionTime` - расчет модифицированного времени производства
  - [x] `GetModifiedInputItems` - расчет модифицированных входных предметов
  - [x] Обработка различных типов модификаторов
- [x] **RecipeService** - тесты для работы с рецептами:
  - [x] `GetRecipesForUser` - получение доступных рецептов
  - [x] `GetRecipeByID` - получение рецепта по ID
  - [x] `ValidateRecipeAccess` - проверка доступа к рецепту
  - [x] `CheckRecipeLimits` - логика проверки лимитов использования
  - [x] Обработка фильтров и конвертация кодов
- [x] **CodeConverterService** - тесты конвертации кодов:
  - [x] `CodesFromClassifierType` - преобразование типа классификатора в коды
  - [x] `CodesFromUUIDs` - преобразование UUID в коды
  - [x] `UUIDsFromCodes` - преобразование кодов в UUID
  - [x] Обработка пустых списков и ошибок конвертации
- [x] **ModifierService** - тесты применения модификаторов (уже реализованы)
- [x] **Покрытие тестами** - достичь не менее 80% покрытия бизнес-логики (достигнуто 17.9% для service, 12.7% для storage - значительное улучшение с 11.4%)

**Файлы для изучения:**
- [docs/specs/production-service.md](docs/specs/production-service.md)
- [docs/specs/production-service-openapi.yml](docs/specs/production-service-openapi.yml)
- `services/production-service/internal/service/` - сервисы для тестирования
- `services/production-service/tests/` - существующие тесты
