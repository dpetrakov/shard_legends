...
**Критерии готовности:**
- [ ] Создана структура проекта в `services/auth-service/` согласно спецификации
- [ ] Настроен `go.mod` с необходимыми зависимостями
- [ ] Реализована конфигурация через переменные окружения
- [ ] Настроено structured logging на английском языке
- [ ] Создан основной `main.go` с graceful shutdown
- [ ] Добавлен базовый `GET /health` эндпоинт
- [ ] Проект компилируется без ошибок и проходит линтинг
...
**Критерии готовности:**
- [ ] Создан пакет `internal/services/telegram.go` с валидацией
- [ ] Реализована функция `ValidateTelegramData()` с проверкой HMAC-SHA256
- [ ] Валидируется срок действия `X-Telegram-Auth-Date` (не старше 24 часов)
- [ ] Обрабатываются все edge cases (отсутствующие поля, невалидные данные)
- [ ] Добавлены unit тесты с покрытием ≥90%
- [ ] Добавлено логирование всех операций валидации
...
## I-1: Docker контейнеризация и API Gateway интеграция
**Роль:** Инфраструктура
**Приоритет:** Низкий
**Статус:** [ ] Готов к выполнению

**Описание:**
Создать Docker контейнер для auth-service и интегрировать его в существующую архитектуру API Gateway.

**Критерии готовности:**
- [ ] Создан оптимизированный `Dockerfile` с multi-stage build
- [ ] Контейнер собирается без ошибок, размер <50MB
- [ ] Обновлен `deploy/dev/docker-compose.yml` с auth-service
- [ ] Настроена интеграция с API Gateway: обновлен `api-gateway/nginx.conf`
- [ ] Добавлен маршрут `/api/auth` → `auth-service:8080/auth`
- [ ] Настроены health checks для Docker контейнера
- [ ] Переменные окружения вынесены в `.env` файлы
- [ ] Сервис запускается и отвечает: `curl http://localhost:9000/auth`
- [ ] Все зависимости доступны из контейнера
...
## I-2: Развертывание на всех средах с мониторингом
**Роль:** Инфраструктура
**Приоритет:** Низкий
**Статус:** [ ] Готов к выполнению

**Описание:**
Развернуть auth-service на всех средах (dev, stage, prod) с настройкой мониторинга и миграций БД.

**Критерии готовности:**
- [ ] Создан контейнер для миграций БД: `migrations/Dockerfile`
- [ ] Обновлены все docker-compose файлы: `deploy/{dev,stage,prod}/docker-compose.yml`
- [ ] Настроено применение миграций перед запуском сервиса
- [ ] Auth-service успешно развернут на всех средах:
  - [ ] DEV: `https://dev.slcw.dimlight.online/api/auth` отвечает
  - [ ] STAGE: `https://stage.slcw.dimlight.online/api/auth` отвечает
  - [ ] PROD: `https://slcw.dimlight.online/api/auth` отвечает
- [ ] Health checks работают на всех средах
- [ ] Логи централизованы и доступны через `docker logs`
- [ ] Настроены алерты на критические ошибки
- [ ] Мониторинг метрик через Prometheus endpoints

**Зависимости:** I-1, T-2, I-0
**Оценка:** 2-3 дня
