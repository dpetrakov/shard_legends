...
### Inventory Service - Разделение серверов и защита от oversell

**Инвентаризация портов: Разделить внутренние и внешние эндпоинты inventory-service**
**Дата завершения:** 2025-06-28
**Роль:** Backend Developer
**Статус:** [x] Выполнена

**Описание:** Реализовано изолированное размещение внутренних и публичных эндпоинтов inventory-service на отдельных портах 8080 (public) и 8081 (internal) по аналогии с auth-service.

**Результат:**
- ✅ **Архитектура серверов:** PublicServer и InternalServer с независимыми роутерами
- ✅ **Маршрутизация:** внутренние (`/reserve`, `/return-reserve`, `/consume-reserve`, `/health`, `/metrics`) доступны только на 8081;
  публичные (`/inventory/*`) — на 8080
- ✅ **Middleware:** проверка Service-JWT с ролью `internal` для внутренних эндпоинтов
- ✅ **Конфигурация:** переменная `INTERNAL_SERVICE_PORT` добавлена в config
- ✅ **Документация:** `docs/specs/inventory-service-openapi.yml` обновлена, указаны два сервера
- ✅ **Тесты:** `test/e2e_server_separation_test.go` подтверждает разделение и авторизацию
...
**Описание:** Устранена критическая уязвимость oversell при конкурентном резервировании предметов.

**Результат:**
- ✅ **SELECT … FOR UPDATE:** блокировка строк `balances` в транзакции резервирования
- ✅ **Триггер-функция и CHECK-constraint:** гарантируют ненулевой остаток (модуль 006 миграция)
- ✅ **Уровень изоляции SERIALIZABLE:** исключает фантомные чтения
- ✅ **Миграция:** `006_add_balance_constraints.up.sql` применена
- ✅ **Конкурентные тесты:** `reservation_race_test.go` (50+ горутин) и доп. тесты изолированности
- ✅ **E2E:** отсутствие oversell подтверждено, ошибки недостаточного баланса корректно обрабатываются
